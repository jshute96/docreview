 ...
 * Uses robust ARIA and data-attribute selectors to avoid brittle minified classes.
 */
(function() {
  ...
  function injectDrive() {
    ...
    rows.forEach(function(row) {
      // Target the Name column (usually data-column-id="16")
      var nameArea = row.querySelector('[data-column-id="16"]') || row;
      
      // Skip if icon already exists in this area
      if (nameArea.querySelector('.dr-link')) return;

      // Find the file ID. Usually on an element with data-id.
      ...
      if (docId && docId.length > 20) {
        // Find the primary type icon to sit next to (first SVG or IMG)
