...
- `getComments` / `getComment(discoId)` — Extracts comment data from an open Google Docs tab by executing `getComments()` in MAIN world. Calls `loadAllComments()` first to ensure resolved/unanchored items are available. Returns status, author, isMine flag, body text/html, full reply threads, and `tabName` extracted from stream view items. Singular form tries without loading first, then retries after `loadAllComments()` if not found.
- `getSuggestions` / `getSuggestion(discoId)` — Extracts suggestion data from an open Google Docs tab by executing `getSuggestions()` in MAIN world. Calls `loadAllComments()` first to ensure resolved items are available. Returns suggestion type, old/new text, status, author, isMine flag, full reply threads, and `tabName` extracted from stream view items. Singular form tries without loading first, then retries after `loadAllComments()` if not found.
- `getCommentsAndSuggestions` — Combined fetch: full suggestions + minimal comment info (`id` + `originalContentDeleted`). Calls `loadAllComments()` first. Used by the comments page on load and refresh to get both in a single round-trip. Both comments and suggestions include `tabName` from stream view headers. Also returns `missingIdCount` — a count of list items dropped because their disco ID couldn't be extracted. Uses the async `fetchCommentsAndSuggestions()`, which retries in-page (3 attempts, 200ms apart, bounded by a deadline that keeps the round-trip inside the bridge's 8s timeout) before reporting; a non-zero count means the scrape is partial and the caller should re-fetch. Items are never given a placeholder ID — see `comment-tracking.md`.
...
**Helper versioning.** `injectDiscoIdHelpers` installs `window.__docreviewDisco` on the Google Docs page and is injected before every MAIN-world call (nine sites in `background.js`). It carries a `VERSION` constant and re-installs when the page's copy is *older* (`>=`, not `===` — with an unpacked dev build and a packed build both loaded, a strict check would make each tear the other down on every call). This matters because a doc tab that was open when the extension reloaded keeps the object the *previous* build put there — a plain "already injected?" check would preserve it forever, so any method added by a newer build would be missing at every call site until the tab was reloaded. **Bump `VERSION` whenever the shape of `window.__docreviewDisco` changes** (methods added, renamed, or re-signatured), or when the behavior of a helper installed by `injectDiscoIdHelpers` changes in a way already-open tabs shouldn't keep the old version of — the early return means an unbumped fix never reaches them. That covers the `window.*` console helpers (`getComment`, `getSuggestion`, `listComments`, …) as well, since they're assigned below the same early return. It isn't free — each open tab pays a teardown + reinstall on its next MAIN-world call, re-discovering the disco ID path and clearing `_loadedSuccessfully` so the next `loadAllComments()` re-opens the pane instead of short-circuiting — so it's a judgment call, not a reflex for cosmetic tweaks. Re-installing calls the old copy's `teardown()` first to disconnect its MutationObservers and clear its polling intervals, so the two builds don't run in parallel. `teardown()` covers the long-lived work only; the short-lived timers inside `loadAllComments` / `waitForSelector` / `waitForPaneSettle` are self-bounded (≤5s) and left alone, except that a `tornDown` flag stops an orphaned in-flight `loadAllComments` from calling `closeCommentsPane()` and yanking the pane out from under the new build's scrape. Builds older than VERSION 2 have no `teardown()`; upgrading from one leaves a single orphaned observer set and interval in already-open tabs, which clears on the next tab reload.
...
7. For suggestion events, after updating DB records, re-scrapes the extension for richer data (replies, author, accepted/rejected status). With a disco ID, fetches just that suggestion via `getSuggestionFromDoc`; without, falls back to `fetchExtensionCommentsAndSuggestions` for all.
...
- **`listComments()`** — dumps all visible comments/suggestions with disco IDs, types, authors, and text. Returns an array of `{ id, type, author, text }` and logs each entry. Only returns items currently in the DOM.
- **`getComments()`** — returns all comments with detailed info: disco ID, status (open/resolved), author, isMine flag, timestamp, comment body text/html, reply threads with author/timestamp/text/html and resolve/reopen actions, and `tabName` from `.streamdocoview-header-prefix-location`. Deduplicates across anchored sidebar and comments pane, preserving header info from stream view entries. Only returns items currently in the DOM — call `loadAllComments()` first to include resolved/unanchored items.
- **`getComment(discoId)`** — returns a single comment by disco ID, or null if not found. Only searches items currently in the DOM.
- **`getSuggestions()`** — returns all suggestions with detailed info: disco ID, suggestion type (Replace/Add/Delete), old/new text, status (open/accepted/rejected), author, isMine flag, timestamp, reply threads with author/timestamp/text/html, and `tabName` from stream view header. Deduplicates across anchored sidebar and comments pane. Only returns items currently in the DOM — call `loadAllComments()` first to include resolved items.
- **`getSuggestion(discoId)`** — returns a single suggestion by disco ID, or null if not found. Only searches items currently in the DOM.
...
- **`loadAllComments()`** — ensures both comment lists are loaded (anchored sidebar + stream view with resolved/unanchored). If the comments pane hasn't been opened yet, waits for the "Show all comments" toolbar button (can lag during slow page loads), opens the pane, then waits for the pane's stream view to stop mutating for 250ms (MutationObserver debounce — Google populates items incrementally, so "any item exists" is not a reliable done signal) before closing. If the pane is already open, waits for settle without closing. On subsequent calls, skips re-opening if a previous call settled successfully and items are still in the DOM. Detects empty docs via the zero-state element. View-only docs have no comment infrastructure and time out after 5s (acceptable — nothing to scrape). Logs start/finish with listitem count to console. Also called internally by background.js message handlers before fetching comments/suggestions.
