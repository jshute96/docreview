...
- `getCommentsAndSuggestions` — Combined fetch: full suggestions + minimal comment info (`id` + `originalContentDeleted`). Calls `loadAllComments()` first. Used by the comments page on load and refresh to get both in a single round-trip. Both comments and suggestions include `tabName` from stream view headers.
