...
- **`loadAllComments()`** — ensures both comment lists are loaded (anchored sidebar + stream view with resolved/unanchored). If the comments pane hasn't been opened yet, opens it to trigger loading, waits for stream items to appear, then closes it. If the pane is already open, waits without closing. No-ops if both lists are already loaded. Detects empty docs (zero-state element) and view-only docs (no comment infrastructure) to avoid unnecessary waits. Logs start/finish to console. Also called internally by background.js message handlers before fetching comments/suggestions.
