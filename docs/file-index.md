...
| `gmail.ts` | Gmail notification scanner — `scanGmailForDocIds(userId, since)` queries Gmail for doc sharing/comment emails after a `Date`, extracts doc IDs via the structured parser and formats share notes via `formatShareNote()`; `scanGmailNotifications` wraps it with Drive metadata fetch; filters by `internalDate` for timestamp-level precision; also exports `isNoGmailMailboxError`, `describeGoogleApiError`, `formatShareNote` |
