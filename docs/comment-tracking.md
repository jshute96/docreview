...
**Active comment count** is shown per doc in the table. A comment is "Active" if it's a thread
you care about that hasn't been resolved (or that was resolved by someone else, meaning it may
need follow-up). A blank cell means no threads currently demand attention.
...
of new Drive activity.

**For all other statuses**, apply this logic:

1. Update `resolved` and `iParticipated` from Drive data.
...
## "I Participated" Detection

Each comment object includes author info and a list of replies:
...
These fields are stored on each `Comment` row and used for future filtering features
(e.g., "show only threads I'm involved in").
