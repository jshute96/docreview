...
- **403/404 are expected outcomes** (file deleted, trashed, or access revoked), not failures: handle them explicitly in every catch block around a Drive/Docs call, log with `logWarning` (never `logError`), and return an empty-but-successful result on read paths / the real 403 or 404 status on write paths, never a blanket 502. A 404 is ambiguous — Drive returns it for both "deleted" and "you can't see this" — so only the refresh route infers `Doc.accessState` from it (see `docs/access-states.md`). Reference implementation: `threads/edit/route.ts` `writeErrorResponse()`.
