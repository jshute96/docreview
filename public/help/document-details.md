...
- **(edited &lt;time&gt;)** after the posted time on any message that was changed after it went up -- whether it was edited here or in Google Docs. Replies are always marked. The first comment of a thread is a special case: Google keeps only one edit time for the whole thread, and each new reply overwrites it, so an edit to the first comment can only be shown when nothing else has happened in the thread since. If someone replied after the edit, the marker isn't shown -- Google no longer has the information. A first comment with no replies always shows it.
